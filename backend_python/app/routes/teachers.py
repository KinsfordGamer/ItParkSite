...
async def get_teachers(current_user: dict = Depends(get_current_user)):
    ...
    teachers = await db.teachers.find(query).sort("createdAt", -1).to_list(None)
    result = []
    for t in teachers:
        t = serialize_doc(t)
        courses = await db.courses.find(
            {"teacher": ObjectId(t["_id"])}, {"title": 1, "isActive": 1}
        ).to_list(None)
        t["courses"] = [serialize_doc(c) for c in courses]
        result.append(t)
